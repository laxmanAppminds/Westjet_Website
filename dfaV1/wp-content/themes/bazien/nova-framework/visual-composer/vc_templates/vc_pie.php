...
$output = "\n\t".'<div class= "'.$css_class.' '.$pie_type.'" data-pie-value="'.$value.'" data-pie-label-value="'.$label_value.'" data-pie-units="'.$units.'" data-pie-color="'.htmlspecialchars($color).'">';
    $output .= "\n\t\t".'<div class="wpb_wrapper">';
	
	if ($title!='') {
        $output .= "\n\t\t\t".'<div class="vc_pie_wrapper with_title">';
	}else{
        ...
        if ($title!='') {
        $output .= '<h4 class="wpb_heading wpb_pie_chart_heading">'.$title.'</h4></div>';
        }else{
			$output .= '</div>';
        }
